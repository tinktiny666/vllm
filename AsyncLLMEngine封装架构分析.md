...
> **注意**: `InputProcessor`、`AsyncMPClient` 和 `OutputProcessor` 都是 `generate` 方法内部调用的组件。`OutputProcessor` 实际上是一个**并行运行的后台任务**。

```text
用户调用: async_llm.generate(prompt, ...)
      │
      ▼
┌──────────────────────────────────────────────────────────────────┐
│  AsyncLLM.generate() 内部逻辑                                     │
│                                                                  │
│  Step 1: 提交请求 (add_request)                                  │
│  ┌────────────────────────────────────────────────────────────┐  │
│  │ 1. InputProcessor.process_inputs()  <-- 翻译 (Text->Token) │  │
│  │ 2. 创建 RequestOutputCollector      <-- 准备"结果桶"       │  │
│  │ 3. AsyncMPClient.add_request_async() <-- 发送 (Via ZMQ)    │  │
│  └────────────────────────────────────────────────────────────┘  │
│            │                                                     │
│            ▼                                                     │
│  Step 2: 循环产出 (AsyncGenerator)                               │
│  ┌────────────────────────────────────────────────────────────┐  │
│  │ while not finished:                                        │  │
│  │    output = await RequestOutputCollector.get()             │  │
│  │    yield output  <──────────────┐ (阻塞等待后台填充)       │  │
│  └────────────────────────────────┘│                         │  │
│                                    │                         │  │
│  ──────────────────────────────────┼──────────────────────── │  │
│  [后台任务] OutputProcessor Loop   │                         │  │
│  ┌─────────────────────────────────┼───────────────────────┐ │  │
│  │ 1. AsyncMPClient.get_output_async() (Via ZMQ 接收)      │ │  │
│  │ 2. OutputProcessor.process_outputs() (Token->Text)      │ │  │
│  │ 3. RequestOutputCollector.put(output) ──────────────────┘ │  │
│  └───────────────────────────────────────────────────────────┘ │
└──────────────────────────────────────────────────────────────────┘
```
